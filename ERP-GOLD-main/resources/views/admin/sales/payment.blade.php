            ...
            <div class="modal-body" id="smallBody">
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input required type="text" id="money" name="money" class="form-control text-center" readonly value="{{ $money }}"/>
                            <input type="hidden" name="type" id="type" value="{{ $type }}">
                            <input type="hidden" name="selected_branch_id" id="selected_branch_id" value="{{ $branchId }}">
                            <input type="hidden" name="visa" id="visa" value="0">
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <label>{{ __('main.cash') }}</label>
                            <input required type="number" id="cash" name="cash" min="0" step="any" class="form-control" placeholder="0" value="{{ number_format((float) $money, 2, '.', '') }}"/>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <label>المتبقي حتى يطابق الإجمالي</label>
                            <input type="text" id="payment_remaining" class="form-control text-center" readonly value="0.00">
                        </div>
                    </div>
                </div>
                ...
                @if ($bankAccounts->isEmpty())
                    <div class="alert alert-warning">
                        لا توجد حسابات بنكية نشطة على هذا الفرع حاليًا. يمكنك حفظ الفاتورة نقديًا فقط، أو تعريف حساب بنكي من إعدادات النظام أولًا.
                    </div>
                @endif

                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h6 class="mb-0">أسطر الدفع غير النقدي</h6>
                    <button type="button" class="btn btn-outline-primary btn-sm" id="add_payment_line_btn" {{ $bankAccounts->isEmpty() ? 'disabled' : '' }}>
                        إضافة سطر بنكي
                    </button>
                </div>

                <div class="table-responsive">
                    <table class="table table-bordered text-center mb-3" id="payment_lines_table">
                ...
                <div class="text-muted mb-3" id="payment_lines_total_wrapper">
                    إجمالي غير نقدي: <strong id="payment_lines_total">0.00</strong>
                </div>
